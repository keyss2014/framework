...
public class MsSqlDataEngine implements DataEngine {
    ...
    private void appendTable(StringBuilder sb, DbTable table) {
        switch (table.getTableType()) {
            case DEFAULT:
                if (table.getSchema() != null && !"".equals(table.getSchema().trim())) {
                    sb.append("[");
        ...
        sb.append("]");
        if (table.getTableType() == DbTableType.DEFAULT) {
    ...
    @Override
    public DataEngine.ParseResult parseDbQuerySelectCountSql(DbQuery query) {
        if (query == null)
        ...
        DataEngine.ParseResult result = new DataEngine.ParseResult();
    ...
    @Override
    public DataEngine.ParseResult parseDbQuerySelectSql(DbQuery query) {
        if (query == null)
            throw new IllegalArgumentException("query对象不能为null！");
        ...
        if (query.getStartIndex() == 0) {
            ...
            if (query.getMaxRecordReturn() != 0) {
                sqlStr.append("TOP ");
                sqlStr.append(query.getMaxRecordReturn());
                sqlStr.append(" ");
            }
            appendSelects(sqlStr, query);
            sqlStr.append(" FROM ");
            ...
            appendSelects(sqlStr, query);
            sqlStr.append(", ROW_NUMBER() OVER ( ");
            appendOrders(sqlStr, query);
            sqlStr.append(" ) __RowNumber FROM ");
            this.appendTables(sqlStr, query);
            this.appendFilters(sqlStr, query, paras);
            sqlStr.append(") A WHERE A.__RowNumber >= :StartIndex AND A.__RowNumber < :EndIndex");
            paras.put("StartIndex", query.getStartIndex() + 1);
            paras.put("EndIndex", query.getStartIndex() + query.getMaxRecordReturn() + 1);
        }
        DataEngine.ParseResult result = new DataEngine.ParseResult();
        result.setDynamicParameters(paras);
        result.setSqlText(sqlStr.toString());
    ...
    private void appendSelects(StringBuilder sb, DbQuery query) {
        if (query == null || query.getSelects() == null || query.getSelects().size() == 0) {
